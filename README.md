...
| | |
|---|---|
| `template/main.rs` | Becomes `src/bin/*.rs` for each new problem |
| `template/copy/` | Anything here is copied into every package as it is. It does not exist until you make it — for a `.vscode/launch.json`, an `.envrc`, a `justfile` |
| `template/dependencies.toml` | The crates a package gets. `env update` regenerates it, showing the diff first |
| `template/Cargo.lock` | Fetched by `env update`; the versions the judge builds with |
| `config.toml` | Where packages go, the profile and parallelism `test` uses, the timeout, whether a problem may be inferred, and so on |
